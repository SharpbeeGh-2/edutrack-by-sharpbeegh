...
const features = [
  { icon: Users, title: 'Student & Admissions', description: 'Complete student lifecycle from admission to graduation' },
  { icon: BookOpen, title: 'Classes & Subjects', description: 'Manage classes, subjects, timetables and assignments' },
  { icon: BarChart3, title: 'Assessments & Exams', description: 'Create exams, record scores, generate report cards' },
  { icon: CreditCard, title: 'Fees & Finance', description: 'Fee structures, payments, receipts and financial reports' },
  { icon: Bell, title: 'Communication', description: 'Announcements, notifications, and messaging to all stakeholders' },
  { icon: FileText, title: 'Report Cards', description: 'Auto-generate professional report cards every term' },
  { icon: Bus, title: 'Transportation', description: 'Manage bus routes and assign students to routes' },
  { icon: Building2, title: 'Hostel Management', description: 'Room allocation, hostel fees and boarding management' },
  { icon: Users, title: 'Staff & Payroll', description: 'Staff profiles, roles, attendance and salary management' },
  { icon: ShieldCheck, title: 'Attendance', description: 'Daily attendance tracking with detailed analytics' },
  { icon: GraduationCap, title: 'Promotion & Alumni', description: 'Promote students and maintain alumni records' },
  { icon: Globe, title: 'Multi-Tenant', description: "Each school gets its own secure, isolated environment" },
]

const steps = [
  { number: '01', title: 'Create Your Account', description: 'Sign up and create your school profile with basic information.' },
  { number: '02', title: 'Configure Your School', description: 'Set academic year, terms, grading scale, classes and subjects.' },
  { number: '03', title: 'Start Managing', description: 'Add students and staff, and start using every module right away.' },
]
...
const plans = [
  ...
  },
  {
    ...
    features: ['Up to 300 students', 'All core modules', '5 admin users', 'Priority support', 'SMS notifications'],
    featured: true,
  },
  {
    name: 'Premium',
    ...
    features: ['Unlimited students', 'All modules', 'Unlimited users', 'Dedicated support', 'Custom branding', 'API access'],
    featured: false,
  },
]
...
function LogoMark({ light = false }: { light?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-h5 font-bold ${light ? 'text-white' : 'text-gray-900'}`}>EduTrack</span>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="px-6 py-16 text-center">
        ...
        <h1 className="mx-auto max-w-3xl text-h1 font-extrabold leading-tight text-gray-900">
          The Complete{' '}
          <span className="text-accent">School Management</span>{' '}
          System for Modern Schools
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-body-lg text-gray-500">
          EduTrack streamlines every aspect of school administration — from admissions to
          alumni — in one powerful, easy-to-use platform.
        </p>
      ...
      <section id="features" className="mx-auto max-w-3xl px-6 py-16">
        <h2 className="text-center text-h2 font-bold text-gray-900">Everything Your School Needs</h2>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {features.map((feature) => (
            <div key={feature.title} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <section className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-10 space-y-6">
          {plans.map((plan) => (
            ...
            >
              <p className={plan.featured ? 'text-body-md text-gray-300' : 'text-body-md text-gray-500'}>
                {plan.name}
              </p>
              <p className="mt-1 flex items-baseline gap-1">
                <span className="text-h1 font-extrabold">{plan.price}</span>
                <span className={plan.featured ? 'text-body-md text-gray-400' : 'text-body-md text-gray-500'}>
                  /mo
                </span>
              </p>
              <p className={plan.featured ? 'mt-1 text-body-md text-gray-300' : 'mt-1 text-body-md text-gray-500'}>
                {plan.description}
              </p>
        ...
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-10 grid grid-cols-2 gap-8 sm:grid-cols-3">
            <div>
              ...
              <a href="#features" className="mt-3 block text-body-sm text-gray-400 hover:text-white">Features</a>
              <a href="#pricing" className="mt-2 block text-body-sm text-gray-400 hover:text-white">Pricing</a>
            </div>
            <div>
              <p className="text-body-md font-semibold text-white">Company</p>
              <a href="#" className="mt-3 block text-body-sm text-gray-400 hover:text-white">About</a>
              <a href="#" className="mt-2 block text-body-sm text-gray-400 hover:text-white">Contact</a>
            </div>
            <div>
              <p className="text-body-md font-semibold text-white">Legal</p>
              <a href="#" className="mt-3 block text-body-sm text-gray-400 hover:text-white">Privacy Policy</a>
              <a href="#" className="mt-2 block text-body-sm text-gray-400 hover:text-white">Terms of Service</a>
            </div>
          </div>
